...
###H2
heat2 <- plot_sig_heatmap(H_hpa_adj,
                          pval_var="Pval", title="Supplemental Figure 1. \n HPA axis biomarkers and child development at Year 2",
                          Outcome="Outcome", Exposure="Exposure",
                          print.est=T, print.ci=F,
                          null=0)

###H3
heat3 <- plot_sig_heatmap(H_sam_adj,
                          pval_var="Pval", title="Supplemental Figure 2. \n SAM axis biomarkers and child development at Year 2",
                          Outcome="Outcome", Exposure="Exposure",
                          print.est=T, print.ci=F,
                          null=0)


ggsave(heat1a, file = here::here("figures/FigureS3_heatmap.png"), height=10, width=8)
ggsave(heat1b, file = here::here("figures/FigureS4_heatmap.png"), height=10, width=8)
ggsave(heat2, file = here::here("figures/FigureS1_heatmap.png"), height=10, width=8)
ggsave(heat3, file = here::here("figures/FigureS2_heatmap.png"), height=10, width=8)
...
#H2
p2 <- ggplot(H_hpa_adj, (aes(x=X, y=point.diff))) + 
  geom_point(size=3) +
  geom_errorbar(aes(ymin=lb.diff, ymax=ub.diff),
                width = 0.75, size = 1) +
  ylim(-.5, 0.4) +
  ...
  coord_flip() +
  labs(y = "Mean difference", x = "Biomarker") +
  ggtitle("Figure 2. HPA axis biomarkers and child development at Year 2") + 
  theme(axis.ticks.x=element_blank(),
        legend.position = "bottom",
        strip.text = element_text(vjust=1),
        axis.text.y=ggtext::element_markdown(),
...
#H3
p3 <- ggplot(H_sam_adj, (aes(x=X, y=point.diff))) + 
  geom_point(size=3) +
  geom_errorbar(aes(ymin=lb.diff, ymax=ub.diff),
                width = 0.75, size = 1) +
  ylim(-.45, 0.4) +
  ...
  coord_flip() +
  labs(y = "Mean difference", x = "Biomarker") +
  ggtitle("Figure 3. SAM axis biomarkers and child development at Year 2") + 
  theme(axis.ticks.x=element_blank(),
        legend.position = "bottom",
        strip.text = element_text(vjust=1),
        axis.text.y=ggtext::element_markdown(),
...
ggsave(p3, file = here::here("figures/Figure3_forest_diff_adj.png"), height=10, width=10)
