...
growth_tbl_flex <- function(name, expo_var, out_var, exposure, outcome, results, results_adj, adj_only=F){
  ...
  if(adj_only){
    ...
    tbl <- data.table(matrix(nrow=0, ncol=14))
    skipped<-F
    for (i in 1:length(exposure)) {
      for (j in 1:length(outcome)) {
        exp <- exposure[i]
    ...
    flextbl <- set_header_labels(flextbl,
                                 values = list("V1" = " ", "V2" = " ", "V3" = " ", "V4" = " ", "V5" = " ",
                                               ...
                                               "V11" = "Predicted Outcome at 25th Percentile", "V12" = "Predicted Outcome at 75th Percentile", "V13" = "Coefficient (95% CI)", "V14" = "P-value"))
    flextbl <- add_header_row(flextbl, values = c("","","","","", "Unadjusted", "Adjusted"), colwidths=c(1,1,1,1,1,5,4))
    flextbl <- add_header_row(flextbl, values = c(name, "Outcome","N","25th Percentile","75th Percentile", "Outcome, 75th Percentile v. 25th Percentile"), colwidths=c(1,1,1,1,1,9))
  }
...
hr_tbl_flex <- function(name, expo_var, out_var, exposure, outcome, results, results_adj, adj_only=F, exp_col_size = 1, out_col_size = 1){
  ...
  if(adj_only){
    ...
    flextbl <- set_header_labels(flextbl,
                                 values = list("V1" = " ", "V2" = " ", "V3" = " ", "V4" = " ", "V5" = " ",
                                               "V6" = "Hazard Ratio (95% CI)", "V7" = "P-value",
                                               "V8" = "Hazard Ratio (95% CI)", "V9" = "P-value"))
    flextbl <- add_header_row(flextbl, values = c("","","","","", "Unadjusted", "Adjusted"), colwidths=c(1,1,1,1,1,2,2))
    flextbl <- add_header_row(flextbl, values = c(name, "Outcome","N","25th Percentile","75th Percentile", "Outcome, 75th Percentile v. 25th Percentile"), colwidths=c(1,1,1,1,1,4))
  }
  flextbl <- hline(flextbl, part="header", border=fp_border(color="black"))
  flextbl <- hline_bottom(flextbl, part="body", border=fp_border(color="black"))
